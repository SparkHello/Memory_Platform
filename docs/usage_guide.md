...
`--host 0.0.0.0` 表示监听所有网卡，局域网手机（iPhone/Kelivo）和 Tailscale 才能访问；只在本机用可改成 `--host 127.0.0.1`。
...
- **搜索只有关键词效果**：`EMBEDDING_API_KEY` 为空时会自动回退关键词检索，属预期行为。

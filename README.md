...
[🚀 开始使用](#-快速开始) · [🧭 为什么选择](#-为什么选择-memory-platform) · [🔌 接入客户端](#-客户端接入) · [📚 文档](#-文档)
...
| **需要更换客户端吗？** | 不需要。客户端继续调用 OpenAI 兼容 `/v1`，或通过 `/mcp` 主动使用记忆与知识库。 |
...
- **接入现有客户端，而不是重做聊天入口**：OpenAI-compatible 自动召回与保存，MCP 负责模型主动搜索和整理。
...
| 继续使用现有 OpenAI 兼容客户端，同时获得本地部署、可审计治理、独立知识库和统一模型路由 | **Memory Platform** |
...
- **自动记忆代理与 MCP**：兼容流式回答、工具调用、多模态消息段和推理字段；支持 `off`、`read`、`read-write` 三种记忆模式和会话分支。

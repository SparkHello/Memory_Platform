...
const REASON_RULES: Array<[RegExp, string]> = [
  [/^本地预过滤：用户文本超过/, "这轮消息超过 64 KiB，没有交给提取模型"],
  [
    /^本地预过滤：/,
    "这轮只是寒暄、提问或代码片段，没有需要长期记住的内容，因此没有调用提取模型"
  ],
  [
    /敏感句子未出站且未明确要求记住/,
    "这句涉及密码、证件或账号，没有发给提取模型；如需保存请在同一句里明确说「记住」"
  ],
  [/敏感句子未出站，用户明确要求记住，已本地保存/, "这句按你的要求在本地直接保存，没有发给提取模型"],
  [
    /敏感原文未发送给远程提取模型/,

...
export function UsagePage({ api }: { api: MemoryApi }) {
  ...
  const visibleDaily = useMemo(
    () => (summary?.daily || []).slice(range === "7" ? -7 : -30),
    [range, summary?.daily]
  );
  ...
  return (
    <div className="page-stack usage-page">
      ...
      {summary && (
        <>
          <UsageOverview totals={summary.totals} />
              ...
              <section className="usage-split">
                ...
                <section className="panel usage-operation-panel">
                  ...
                  <dl className="usage-coverage-list">
                    <div>
                      <dt>已精确计费</dt>
                      <dd>{summary.totals.priced_calls} 次</dd>
                    </div>
                    <div>
                      <dt>上游未返回 usage</dt>
                      <dd>{summary.totals.unmeasured_calls} 次</dd>
                    </div>
                    <div>
                      <dt>暂无价格映射</dt>
                      <dd>{summary.totals.unpriced_calls} 次</dd>
                    </div>
                  </dl>
          ...
          <details className="panel usage-pricing">
            <summary>
              <span>
                <strong>当前价格表</strong>
                <small>官方公开 API 原价 · 截至 {summary.pricing.as_of}</small>
              </span>
              <span>{summary.pricing.models.length} 个价格项</span>
            </summary>
            <div className="usage-price-list">
              {summary.pricing.models.map((price) => (
                <div key={price.key}>
                  <span>
            ...
            <p className="usage-pricing-note">{summary.pricing.note}</p>
          </details>
        </>
      )}
    </div>
